<script lang="ts">
  ...
  function scrollToHighlight() {
    ...
    setTimeout(() => {
      ...
      if (element) {
        ...
        element.style.background = 'var(--gold-ghost)';
        element.style.boxShadow = '0 0 0 2px var(--gold)';
        setTimeout(() => {
  ...
  async function restoreFromJson(event: Event) {
    ...
    try {
      ...
      for (const item of data.items) {
        ...
        if (result.success) {
          ...
          const updateRes = await fetch(
            `/api/items/${item.id || item.itemCode}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
      ...
      await loadItems();
      addNotification(
        `Imported ${imported} items, updated ${updated} items`,
        "success",
      );
      showBackupModal = false;
    } catch (error) {
    ...
    <div class="stat-card">
      ...
      <div class="stat-info">
        <span class="stat-value"
          >{formatCurrency(totalValue, $settings.currency)}</span
        >
  ...
  <div class="card">
    <div class="flex flex-col sm:flex-row gap-4 flex-wrap">
      <div class="flex-1 min-w-[200px] relative">
        ...
        {#if searchQuery}
          <button
            ...
            class="absolute right-3 top-1/2 -translate-y-1/2 p-1" style="color: var(--ink-faint)"
      ...
      {#if searchQuery || categoryFilter}
        <button on:click={clearFilters} class="btn btn-secondary shrink-0">
          <svg
            class="w-4 h-4 mr-1"
            fill="none"
            stroke="currentColor"
      ...
      {/if}
      <div class="text-sm shrink-0 ml-auto flex items-center" style="color: var(--ink-faint)">
        <span class="font-semibold" style="color: var(--ink)">{filteredItems.length}</span>
        <span class="mx-1">of</span>
        <span class="font-semibold" style="color: var(--ink)">{items.length}</span>
        ...
        <table class="table">
          ...
          <tbody>
            {#each filteredItems as item}
              ...
              <tr
                id="item-{item.id}"
                class="transition-colors {isHighlighted
                  ? 'ring-2 ring-amber-400/30 ring-inset'
                  : ''}"
                style={isHighlighted ? "background: rgba(245,158,11,0.06)" : ""}
              >
                <td>
                  <div class="flex items-center gap-3">
                    <div
                      class="w-10 h-10 rounded-lg flex items-center justify-center"
                      style="background: rgba(196,30,58,0.08); border: 1.5px solid rgba(196,30,58,0.15);"
                    >
                      <svg
                    ...
                    <div>
                      <p class="font-medium" style="color: var(--ink)">{item.name}</p>
                      <p class="text-sm font-mono" style="color: var(--ink-faint)">
                        {item.itemCode}
                      </p>
                    </div>
                  </div>
                ...
                <td class="font-medium"
                  >{formatCurrency(item.price, $settings.currency)}</td
                >
                <td>
                  <div class="flex items-center gap-2">
      ...
      <form on:submit|preventDefault={saveItem} class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-6">
          ...
          <div class="form-group">
            ...
            <input
              ...
              class="input font-mono {formErrors.itemCode
                ? 'border-red-500'
                : ''}"
          ...
          <div class="form-group">
            ...
            <div class="currency-input-wrapper">
              <span class="currency-input-prefix"
                >{currencySymbol($settings.currency)}</span
              >
              <input
                type="number"
                id="price"
                ...
                class="input {currencySymbol($settings.currency).length > 1
                  ? 'currency-input-long'
                  : 'currency-input-short'} {formErrors.price
                  ? 'border-red-500'
                  : ''}"
                placeholder="0.00"
                disabled={isSubmitting}
          ...
          <div class="form-group">
            ...
            <div
              class="input"
              style="background: var(--paper-warm); color: var(--ink-faint); border: 1px solid var(--border-ink)"
            >
              Piece
            </div>
        ...
        {#if formData.price > 0 && formData.stockQuantity > 0}
          <div
            ...
            style="background: rgba(196,30,58,0.04); border: 2px solid rgba(196,30,58,0.1); border-radius: 6px 8px 6px 7px"
          >
            <div class="flex justify-between items-center">
              <span class="text-sm" style="color: var(--ink-faint)">Total Stock Value</span>
              <span class="text-xl font-semibold" style="color: var(--ink)">
                {formatCurrency(
        ...
        <div
          ...
          style="border-top: 2px dashed var(--border-ink)"
        >
          <button
            type="button"
            on:click={closeModal}
    ...
    <div class="modal max-w-md w-full" on:click|stopPropagation>
      ...
      <div class="p-6 space-y-6">
        ...
        <div class="backup-section">
          ...
          <h3 class="text-lg font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive; font-size: 1.4rem;">Export Inventory</h3>
          <p class="text-sm" style="color: var(--ink-faint)">
            Download all inventory data as a JSON file for backup or transfer.
          </p>
          <button
          ...
          {#if items.length > 0}
            <p class="text-xs text-center mt-2" style="color: var(--ink-faint)">
              {items.length} items • {formatCurrency(
                totalValue,
                $settings.currency,
              )} total value
            </p>
        ...
        <div class="relative">
          ...
          <div class="relative flex justify-center text-sm">
            <span
              class="px-2" style="color: var(--ink-faint); background: var(--paper-card)">or</span
            >
          </div>
        </div>
        ...
        <div class="backup-section">
          ...
          <h3 class="text-lg font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive; font-size: 1.4rem;">Import Inventory</h3>
          <p class="text-sm" style="color: var(--ink-faint)">
            Restore inventory from a previously exported JSON file.
          </p>
