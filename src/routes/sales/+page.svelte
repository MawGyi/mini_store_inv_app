<script lang="ts">
  ...
  $: totalAmount = newSaleForm.items.reduce(
    (sum, item) => sum + item.totalPrice,
    0,
  );
  $: itemCount = newSaleForm.items.reduce(
        ...
        const backupData = {
          ...
          summary: {
            ...
            totalRevenue: data.data.reduce(
              (sum: number, s: Sale) => sum + s.totalAmount,
              0,
            ),
  ...
  function addItemToSale() {
    ...
    const existingIndex = newSaleForm.items.findIndex(
      (i) => i.itemId === itemId,
    );
    if (existingIndex >= 0) {
      const newQty =
        newSaleForm.items[existingIndex].quantity + selectedQuantity;
  ...
  function updateItemQuantity(index: number, newQty: number) {
    ...
    if (item && newQty <= item.stockQuantity) {
      ...
      newSaleForm.items[index].totalPrice =
        newQty * newSaleForm.items[index].unitPrice;
      newSaleForm.items = newSaleForm.items;
    } else {
      addNotification(
        `Maximum ${item?.stockQuantity || 0} units available`,
        "error",
      );
    }
  }
    ...
    <div class="stat-card">
      ...
      <div class="stat-info">
        <span class="stat-value"
          >{formatCurrency(todayTotal, $settings.currency)}</span
        >
    ...
    {:else}
      <div class="table-container">
        <table class="table">
          ...
          <tbody>
            {#each sales as sale}
              <tr class="hover:bg-transparent transition-colors">
                <td>
                  <span class="font-mono text-sm font-medium"
                    >{sale.invoiceNumber}</span
                  >
                </td>
                ...
                <td>
                  <span
                    class="badge {sale.paymentMethod === 'cash'
                      ? 'badge-success'
                ...
                <td class="font-semibold"
                  >{formatCurrency(sale.totalAmount, $settings.currency)}</td
                >
                <td class="text-right">
                  <button
                    on:click={() => viewSaleDetails(sale)}
                    class="text-sm font-medium" style="color: var(--crimson)"
                  >
                    View Details
                  </button>
                </td>
      ...
      {#if totalPages > 1}
        <div
          class="flex items-center justify-between mt-6 pt-4" style="border-top: 2px dashed var(--border-ink)"
        >
...
{#if showNewSaleModal}
  <div
    ...
    on:keydown={(e) => e.key === "Escape" && closeNewSaleModal()}
  >
    <div class="modal max-w-2xl w-full" on:click|stopPropagation>
      <div class="modal-header">
        <h2 class="text-xl font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive; font-size: 1.5rem;">New Sale</h2>
        <button
          on:click={closeNewSaleModal}
          class="btn-icon"
          aria-label="Close"
      ...
      <div class="p-6 space-y-5">
        ...
        <!-- Add Items Section -->
        <div class="rounded-xl p-4" style="border: 2px solid var(--border-ink); background: var(--paper-warm); border-radius: 6px 8px 6px 7px">
          <label class="form-label mb-3">Add Items</label>

          <div class="flex gap-3 mb-3">
            <div class="flex-1">
              <select bind:value={selectedItemId} class="input">
          ...
          {#if selectedItemId}
            ...
            {#if selectedItem}
              <div
                class="flex items-center justify-between p-3 rounded-lg" style="background: var(--paper-warm); border: 2px solid var(--border-ink); border-radius: 6px 8px 6px 7px"
              >
                <div class="flex items-center gap-3">
                  <div
                    class="w-10 h-10 rounded-lg flex items-center justify-center" style="background: rgba(196,30,58,0.06); border: 1px solid rgba(196,30,58,0.12)"
                  >
                    <svg
        ...
        <!-- Items List -->
        {#if newSaleForm.items.length > 0}
          <div class="rounded-xl overflow-hidden" style="border: 2px solid var(--border-ink); border-radius: 6px 8px 6px 7px">
            <table class="table">
              <thead>
                <tr>
                  <th>Item</th>
                ...
                {#each newSaleForm.items as item, index}
                  <tr>
                    ...
                    <td class="font-medium"
                      >{formatCurrency(item.totalPrice, $settings.currency)}</td
                    >
                    <td class="text-right">
                      <button
                        type="button"
              ...
              <tfoot>
                <tr class="bg-transparent">
                  ...
                  <td
                    colspan="2"
                    class="font-bold text-lg py-3 px-4 text-right"
                  >
        ...
        {:else}
          <div class="text-center py-8" style="color: var(--ink-faint)">
            <svg
              class="w-12 h-12 mx-auto mb-3" style="color: var(--ink-faint)"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <!-- Actions -->
        <div class="flex gap-3 pt-4" style="border-top: 2px dashed var(--border-ink)">
          <button
            type="button"
            on:click={closeNewSaleModal}
          ...
          >
            {#if isSubmitting}
              <svg
                class="animate-spin w-5 h-5 inline mr-2"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
...
{#if showDetailModal && selectedSale}
  ...
  >
    <div class="modal max-w-lg w-full" on:click|stopPropagation>
      <div class="modal-header">
        <h2 class="text-xl font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive; font-size: 1.5rem;">Sale Details</h2>
      ...
      <div class="p-6 space-y-5">
        <!-- Sale Info -->
        <div class="grid grid-cols-2 gap-4 p-4 rounded-xl" style="background: var(--paper-warm); border: 2px solid var(--border-ink); border-radius: 6px 8px 6px 7px">
          <div>
            <p class="text-sm" style="color: var(--ink-faint)">Invoice Number</p>
            <p class="font-mono font-medium">{selectedSale.invoiceNumber}</p>
          </div>
          <div>
            <p class="text-sm" style="color: var(--ink-faint)">Date</p>
            <p class="font-medium">{formatDate(selectedSale.saleDate)}</p>
          </div>
          <div>
            <p class="text-sm" style="color: var(--ink-faint)">Customer</p>
            <p class="font-medium">
              {selectedSale.customerName || "Walk-in Customer"}
            </p>
          </div>
          <div>
            <p class="text-sm" style="color: var(--ink-faint)">Payment Method</p>
            <span
              class="badge {selectedSale.paymentMethod === 'cash'
                ? 'badge-success'
        ...
        <!-- Items -->
        <div>
          <h3 class="font-medium mb-3" style="color: var(--ink)">
            ...
            <table class="table">
              ...
              <tbody>
                {#each selectedSale.items as item}
                  <tr>
                    <td class="font-medium"
                      >{item.itemName || `Item #${item.itemId}`}</td
                    >
              ...
              <tfoot>
                <tr class="bg-transparent">
                  <td class="text-right font-semibold py-3 px-4"
                    >Total Amount:</td
                  >
                  <td
                    colspan="2"
        ...
        <div class="flex gap-3 pt-4" style="border-top: 2px dashed var(--border-ink)">
          ...
          <button on:click={closeDetailModal} class="btn btn-primary flex-1"
            >Close</button
          >
        </div>
      </div>
...
{#if showBackupModal}
  ...
  >
    <div class="modal max-w-md w-full" on:click|stopPropagation>
      <div class="modal-header">
        <h2 class="text-xl font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive; font-size: 1.5rem;">
          <svg
      ...
      <div class="p-6">
        <div class="backup-section">
          ...
          <h3 class="text-lg font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive; font-size: 1.4rem;">Export Sales</h3>
          <p class="text-sm" style="color: var(--ink-faint)">
            Download all sales data as a JSON file for backup or analysis.
          </p>
          <button
          ...
          {#if sales.length > 0}
            <p class="text-xs text-center mt-2" style="color: var(--ink-faint)">
              {sales.length} transactions • {formatCurrency(
...
<style>
  .backup-section {
    ...
    background-image: url("data:image/svg+xml,%3Csvg width='20' height='20' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M0 0L20 20M20 0L0 20' stroke='%231a1714' stroke-width='0.15' opacity='0.03'/%3E%3C/svg%3E");
  }
  ...
  .backup-icon {
    ...
    box-shadow: 2px 2px 0 rgba(26,23,20,0.1);
  }
