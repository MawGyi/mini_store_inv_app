<script lang="ts">
  ...
  async function saveSettings() {
    ...
    if (lowStockThreshold !== $settings.lowStockThreshold)
      changes.push("threshold");
    if (enableNotifications !== $settings.enableNotifications)
      changes.push("notifications");
    if (enableEmailReports !== $settings.enableEmailReports)
      changes.push("email reports");
    ...
    if (changes.length === 0) {
      ...
      addNotification(
        `${changes.map(capitalize).join(", ")} updated`,
        "success",
      );
    } else {
  ...
  async function confirmImport() {
    ...
    try {
      ...
      if (data.error) {
        ...
        addNotification(
          `Imported ${data.imported} items successfully${data.skipped > 0 ? ` (${data.skipped} skipped)` : ""}`,
          "success",
        );
...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="card">
    <h2 class="card-header flex items-center gap-2">
      <svg
        class="w-5 h-5" style="color: var(--ink-faint)"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
  ...
  <div class="card">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="form-group">
        ...
        <div class="p-3 rounded-lg" style="background: var(--paper-warm); border: 2px solid var(--border-ink)">
          <span class="text-2xl font-bold" style="color: var(--ink)">{previewAmount}</span>
          <p class="text-sm mt-1" style="color: var(--ink-faint)">Sample price display</p>
        </div>
      </div>
      <div class="form-group">
  ...
  <div class="card">
    ...
    <div class="space-y-4">
      <label
        class="flex items-center justify-between p-4 rounded-xl cursor-pointer" style="background: var(--paper-warm); border: 1px solid var(--border-ink)"
      >
        <div class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-lg flex items-center justify-center" style="background: var(--gold-ghost); border: 1px solid rgba(201,149,44,0.15)"
          >
            <svg
              class="w-5 h-5" style="color: var(--gold-dark)"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div>
            <p class="font-medium" style="color: var(--ink)">Enable Notifications</p>
        ...
        <input
          ...
          class="w-5 h-5 rounded" style="color: var(--crimson)"
        />
      </label>
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-lg flex items-center justify-center" style="background: var(--crimson-ghost); border: 1px solid rgba(184,28,46,0.12)"
          >
            <svg
          ...
          <div>
            <p class="font-medium" style="color: var(--ink)">Email Reports</p>
            <p class="text-sm" style="color: var(--ink-faint)">
        ...
        <input
          type="checkbox"
          bind:checked={enableEmailReports}
          class="w-5 h-5 rounded" style="color: var(--crimson)"
        />
      </label>
  ...
  <div class="card">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div class="data-card">
        ...
        <h3 class="font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive">Full Backup</h3>
        <p class="text-sm mt-1" style="color: var(--ink-faint)">
          Export all inventory, sales, and categories as JSON
        </p>
      ...
      <div class="data-card">
        ...
        <h3 class="font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive">Export Items</h3>
        <p class="text-sm mt-1" style="color: var(--ink-faint)">
          Download all items as CSV spreadsheet
        </p>
      ...
      <div class="data-card">
        ...
        <h3 class="font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive">Export Sales</h3>
        <p class="text-sm mt-1" style="color: var(--ink-faint)">Download sales by date range</p>
    ...
    <!-- Import Section -->
    <div class="mt-6 pt-6" style="border-top: 2px dashed var(--border-ink)">
      <h3 class="text-sm font-medium mb-4" style="color: var(--ink)">Import Items</h3>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="import-section">
          <p class="text-sm mb-3" style="color: var(--ink-faint)">
            Upload a CSV file with columns: name, itemcode, price,
            stockquantity, category, lowstockthreshold
          </p>
          ...
          {#if importFile}
            <p class="text-sm mt-2 flex items-center gap-1" style="color: var(--success)">
              <svg
                class="w-4 h-4"
                fill="none"
                stroke="currentColor"
        ...
        <div class="rounded-xl p-4" style="background: var(--gold-ghost); border: 2px solid rgba(201,149,44,0.15); border-radius: 6px 8px 6px 7px">
          <h4 class="font-medium flex items-center gap-2" style="color: var(--ink)">
            <svg
              class="w-5 h-5"
              fill="none"
              stroke="currentColor"
          ...
          <ul class="text-sm mt-2 space-y-1" style="color: var(--ink-faint)">
      ...
      <div class="danger-zone">
        <div class="flex items-start gap-4">
          <div
            ...
            style="background: var(--crimson-ghost); border-radius: var(--radius-sketchy); border: 2px solid rgba(184,28,46,0.2)"
          >
            <svg
              class="w-6 h-6"
          ...
          <div class="flex-1">
            <h3 class="font-semibold" style="color: var(--crimson)">Danger Zone</h3>
            <p class="text-sm mt-1" style="color: var(--ink-faint)">
              Permanently delete all data including items, sales, and
...
{#if showImportModal}
  ...
  >
    <div
      class="rounded-xl max-w-2xl" style="background: var(--paper-card); border: 2px solid var(--ink) w-full max-h-[90vh] overflow-hidden"
    >
      <div class="flex items-center justify-between p-4 border-b">
        ...
        <button
          ...
          class="" style="color: var(--ink-faint)"
        >
          <svg
            class="w-6 h-6"
            fill="none"
      ...
      <div class="p-4 overflow-y-auto max-h-[60vh]">
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y">
            {#each importPreview as row}
              {@const hasError =
                !row.name || !row.itemcode || !row.price || !row.stockquantity}
              <tr class={hasError ? "bg-red-50" : ""}>
                <td class="px-3 py-2" style="color: var(--ink-faint)">{row._rowNum}</td>
                <td
                  class="px-3 py-2 {hasError && !row.name
                    ? 'text-red-600 font-medium'
                    : ''}">{row.name || "—"}</td
                >
                <td
                  class="px-3 py-2 {hasError && !row.itemcode
                    ? 'text-red-600 font-medium'
                    : ''}">{row.itemcode || "—"}</td
                >
                <td
                  class="px-3 py-2 text-right {hasError &&
                ...
                <td
                  class="px-3 py-2 text-right {hasError &&
                  (!row.stockquantity || isNaN(parseInt(row.stockquantity)))
                    ? 'text-red-600 font-medium'
                    : ''}">{row.stockquantity || "—"}</td
                >
        ...
        {#if importPreview.length === 0}
          <p class="text-center py-8" style="color: var(--ink-faint)">No data to preview</p>
        {/if}
      </div>

      <div class="flex justify-end gap-3 p-4 " style="border-top: 2px dashed var(--border-ink); background: var(--paper-warm)">
        <button
          on:click={() => {
            showImportModal = false;
            importFile = null;
...
{#if showImportModal}
  <div
    class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
  >
    <div class="rounded-xl max-w-md" style="background: var(--paper-card); border: 2px solid var(--ink) w-full">
      <div class="p-6">
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 class="text-lg font-semibold" style="color: var(--ink); font-family: 'Caveat Brush', cursive">Clear All Data</h3>
            <p class="text-sm" style="color: var(--ink-faint)">This action cannot be undone.</p>
          </div>
        </div>

        <p class="text-sm mb-4" style="color: var(--ink-faint)">
          This will permanently delete all items, sales, categories, and
          settings. Your account will remain but all data will be lost.
        </p>

        <label class="block mb-2 text-sm font-medium" style="color: var(--ink)">
          Type <code class="px-1 rounded" style="background: var(--paper-warm); border: 1px solid var(--border-ink)">DELETE</code> to confirm
        </label>
        <input
          type="text"
...
<style>
  ...
  .danger-zone {
    background: var(--crimson-ghost);
    border: 2px solid rgba(184, 28, 46, 0.15);
    border-radius: var(--radius-sketchy-lg);
    padding: 1.25rem;
  }
